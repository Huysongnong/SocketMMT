...
def serverSignup(conn):
    client_newaccount = recvList(conn)

    cursor.execute("insert into Account(username,pass) values(?,?);",(client_newaccount[0],client_newaccount[1]))
    cursor.commit()
...
conx = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};\
                        SERVER=LAPTOP-U92LVGTP; Database=SocketAccount;\
                        UID=h; PWD=123456;')
cursor = conx.cursor()
